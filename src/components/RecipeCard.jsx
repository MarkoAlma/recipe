...
const RecipeCard = ({id, name, steps, ingredients, categ, url, delete_url}) => {
  ...
  return (
    <div style={{flexBasis:'24%', backgroundColor:'rgb(250,250,250) ', boxShadow:'1px 1px 5px 2px lightgray', borderRadius:'10px', position:'relative', padding:'10px'}}>
      <h1>{name}</h1>
        <h6>({categ})</h6>
    ...
    <ul>
        ...
        {ingredients.map(obj => 
            <li style={{textAlign:'left'}}>{obj}</li>
        )}
    </ul>
    <p style={{textAlign:'left'}}>{steps}</p>
